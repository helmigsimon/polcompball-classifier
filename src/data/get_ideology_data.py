...
def convert_page_to_ideology_set(url):
    ...
    for category in tqdm(categories):
        ...
        category_list = list(
            filter(lambda x: x is not None, map(get_ideo, ideology_collection))
        )
...
def get_ideology(idea, **kwargs):
    ...
    ideology_link = [x for x in idea.findAll("a") if filter_ideology_links(x)][-1].get(
        "href"
    )
    if ideology_link:
    ...
    return Ideology(
        name=ideology_name, page_url=page_url, thumbnail_url=thumbnail_url, **kwargs
    )
...
def combine_same_ideologies(balls):
    ...
    multiple_balls = {
        name: list(filter(lambda x: x.name == name, balls))
        for name in multiple_time_names
    }
    ...
    combined_ideologies = [
        *combined_balls,
        *[ball for ball in balls if ball.name not in multiple_balls.keys()],
    ]
